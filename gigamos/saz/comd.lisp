...
and then to the minibuffer command before that one, and so on." ()
  (unless (boundp '*OUTER-LEVEL-MINI-BUFFER-COMMAND*)
    (edit-in-mini-buffer)
  ...
  (COND ((ZEROP LEN)
        ...
        ((OR (< LEN 50.)
         ...
         (SEND *STANDARD-OUTPUT* :ITEM-LIST 'COMPLETION
               (mapcar #'(lambda (item) (if (= (length item) 1) ;a cons, not a list
                                            (first item)
                                          (string-append (first item) (string #/TAB) (third item))))
                       (SORT poss #'(lambda (item1 item2) (STRING-LESSP (car item1) (car item2))))))
         (TERPRI *STANDARD-OUTPUT*))))
