...
(tv:add-typeout-item-type *item-type-alist* frame "Introduction to Window Maker"
                          :help t "Provide help on a topic." fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* frame "Split Pane Horizontally"
                          :horizontal-split nil "Split pane under mouse horizontally" fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* frame "Split Pane Vertically"
                          :vertical-split nil "Split pane under mouse vertically" fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* frame "Delete Pane" :kill nil
                          "Remove pane under mouse from display; an adjacent pane will grow into this pane's space. "
                          fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* frame "Define Properties"
                          :define-properties nil "Set default init plist properties for pane under mouse." fonts:cptfontb)
...
(tv:add-typeout-item-type *item-type-alist* pane "Introduction to Window Maker"
                          :help t "Provide help on a topic." fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* pane "Split Pane Horizontally"
                          :horizontal-split nil "Split pane under mouse horizontally" fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* pane "Split Pane Vertically"
                          :vertical-split nil "Split pane under mouse vertically" fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* pane "Delete Pane" :kill nil
                          "Remove pane under mouse from display; an adjacent pane will grow into this pane's space. "
                          fonts:cptfontb)
(tv:add-typeout-item-type *item-type-alist* pane "Define Properties"
                          :define-properties nil "Set default init plist properties for pane under mouse." fonts:cptfontb)
      ...
      (cond ((zerop number-of-elements) (if owner (funcall owner ':fix-yourself self)
            ...
            ((= number-of-elements 1)
             ...
             (if (typep (first list-of-panes-or-frames) 'pane)
               ...
               ;; otherwise the element is a frame that should not exist any more (no use for it).
            ...
            (t (if (= (length lines-bordering-it) 1)
                   ...
                   ;;
                   (setq window-or-windows-to-expand
                         (funcall (first lines-bordering-it) ':get-other-window pane-or-frame-killed))
                 (let ((user-choice-list-function
                         (or (tv:menu-choose
                               (if (equal direction-of-slice ':horizontal)
                                   '(("Top Neighbor" :eval 'first :font fonts:cptfontb
                                      :documentation "Top neighbor expands downwards into the space.")
                                     ("Bottom Neighbor" :eval 'second :font fonts:cptfontb
                                      :documentation "Bottom neighbor expands upwards into the space.")
                                     ("Both Neighbors" :eval 'list :font fonts:cptfontb
                                      :documentation "Both neighbors get half of the space."))
                                 '(("Left Neighbor" :eval 'first :font fonts:cptfontb
                                    :documentation "Left neighbor expands /"east/" into the space.")
                                   ("Right Neighbor" :eval 'second :font fonts:cptfontb
                                    :documentation "Right neighbor expands /"west/" into the space.""))
                                                    ("Both Neighbors" :list 'eval :font fonts:cptfontb
                                                     :documentation "Both neighbors get half of the space."")))
                               '(:string "Which neighbor gets its space?" :font fonts:cptfontb))
                             'eval)))
                 (setq window-or-windows-to-expand (funcall user-choice-list-function
                                                            (get-windows-bordered-by-lines
                                                              lines-bordering-it
                                                              pane-or-frame-killed direction-of-slice)))
                 (typecase window-or-windows-to-expand
                   (:symbol
                   ...
                   (:list
                    ;;  We want to expand both windows
                    (funcall (first window-to-expand) ':expand-into-half pane-or-frame-killed)
                    (funcall (second window-to-expand) ':expand-into-half pane-or-frame-killed))
                   (otherwise (ferror nil "Unable to expand windows into deleted window's space."))))))))))
;;
;;
