...
(DEFFLAVOR new-WINDOW-MAKER-frame
  ...
  (:default-init-plist
    ...
    '((title-pane tv:window
      ...
      (plist-properties-pane zwei:zmail-choose-variable-values-pane :label "Default Properties" :blinker-p nil :save-bits t))
    :constraints
    '((main . ((title-and-instrument-and-menu-pane graphics-and-plist-properties-pane)
               ...
               ((graphics-and-plist-properties-pane :horizontal (0.9)
                                                     (graphics-pane plist-properties-pane)
                                                     ((graphics-pane .733))
                                                     ((plist-properties-pane :even))))))))
  :INITTABLE-INSTANCE-VARIABLES
  :GETTABLE-INSTANCE-VARIABLES
  :SETTABLE-INSTANCE-VARIABLES
...
(DEFUN PROCESS-FUNCTION (window)
  (let* ((*window-maker* window)
         ...
         (*plist-properties-pane* (funcall window :get-pane 'plist-properties-pane))
...
(defmethod (window-maker-frame :after :refresh) (&optional type)
  (let* ((*window-maker* self)
         ...
         (*plist-properties-pane* (funcall self :get-pane 'plist-properties-pane))
          ...
          (selectq (car blip)
            ...
            (:typeout-execute
             (selectq (second blip)
               ...
               (:help (tv:menu-choose help-message '(:string "Using Window Maker" :font fonts:metsi :centered)))
               (:horizontal-split (funcall object :slice :horizontal))
               (:insert-new-pane (funcall object :insert-new-pane))
               (:define-properties (funcall object :define-properties))
