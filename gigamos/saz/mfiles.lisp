...
(DEFUN GET-NEW-MAIL-FOR-ZMAIL-BUFFER (ZMAIL-BUFFER FROM-FILE DELETE-P TELL-BACKGROUND-P
  ...
  (IF (NOT MOVE-P)
    ...
    (LOCK-ZMAIL-BUFFER (*ZMAIL-BUFFER*)         ;Don't allow getting all messages
          (IF (AND APPEND-P (EQ *ZMAIL-BUFFER* ZMAIL-BUFFER))
              (progn (SETQ *MSG-NO*  (- (print (array-active-length *zmail-buffer*))
                                        (print (ZMAIL-BUFFER-NMSGS *ZMAIL-BUFFER*))))   ;but may have already.
                     (ZMAIL-SELECT-MSG *msg-no*))
