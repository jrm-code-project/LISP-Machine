...
(DEFUN LOAD-ZMAIL-INIT-FILE (&OPTIONAL SILENT file)
  (UNLESS (EQUAL *ZMAIL-INIT-LOADED* USER-ID)
    (WITH-LOCK (*ZMAIL-PROFILE-LOADING-LOCK-CELL*)
      (CONDITION-CASE ()
          (WITH-OPEN-FILE (STREAM (or file (ZMAIL-INIT-FILE-PATHNAME))
                                  :CHARACTERS :DEFAULT)
        ...
        (fs:directory-not-found
         ;;check lispm file server in fs:user-id's home dir
         (load-zmail-init-file silent (make-pathname :defaults fs:load-pathname-defaults
                                                     :name "ZMAIL.INIT")))
        (FS:FILE-NOT-FOUND
         NIL)
