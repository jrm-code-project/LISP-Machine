               ...
               (T
                (loop for i from 0
                      ...
                      (when (zerop (mod i 16))
                        (outi-for-k `(k:open)))
                      (p2-for-k arg reg))
                (loop for i below nargs downto 0
                      for reg = (o-n (mod i 16))
                      for rreg = (k-find-return-register i)
                      do
                      (outi-for-k `(k:move ,rreg ,reg k:boxed-right))
                      (when (zerop (mod i 16))
                        (outi-for-k `(k:call (li::discard-open-frame 0)))))
                (outi-for-k `(k:movei ,(k-find-return-register-count) ,nargs))
                (setq *drop-through* nil))))
        ((NUMBERP *M-V-TARGET*)
...
(DEFUN GOTAGS-SEARCH (TAG)
  (OR (SI:ASSOC-EQUAL TAG *GOTAG-ENVIRONMENT*)
...
(DEFUN GPDLLVL (TAG)
  ...
  (LET ((GOTAG (SI:ASSOC-EQUAL TAG *GOTAG-ENVIRONMENT*)))
