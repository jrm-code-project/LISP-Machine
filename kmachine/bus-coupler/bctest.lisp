...
;;; Bus coupler diagnostic tests        9-01-88       Rich R.

(defun start-bc-test ()
(format t "~%                               GIGAMOS BUS COUPLER TEST.~10%")
(format t "~%                               This test runs a memory diagnostic.~10%")
(format t "~%                               from a Lambda to a remote system.~10%")
(format t "~%                               (either a Mac II or another Lambda.~10%")
(defun LOCAL?-L ()
  (SETQ LOCAL-L (prompt-and-read ':read "~%What is the Local System?  Lambda or Mac II."))
    (format terminal-io "THE LOCAL SYSTEM IS  ~a." LOCAL-L))
