...
(defsubst pkg-code-hash-code (code)
  (hw:ldb code (byte (1- (byte-size vinc:%%fixnum-field)) 0) 0))
...
Returns NIL if there is no such package."
  ...
  (cond ((package-p name) name)
        ...
        (t (or (stringp name) (symbolp name))
  ...
  (or (and (package-p thing) thing)
      ...
      (case create-p
        ...
        ((nil :error) ;remove the paren if below is restored
         (error "No package named ~a found" thing))
...
(defun make-package (package-name &key nicknames (use '("LISP")))
...
(defun pkg-rehash (pkg &optional (size (ash (pkg-max-number-of-symbols pkg) 1)))
  (let ((table-size (pkg-good-size size)))
    (let ((new-hash-table-codes (array:make-vector table-size))
          (new-hash-table-symbols (array:make-vector table-size)))
      (do-local-symbols (sym pkg nil code)
        (let ((hash (pkg-string-hash-code (string sym))))
          (rehash-intern-store hash sym table-size
                               new-hash-table-codes
      ...
      (setf (pkg-number-of-slots pkg) table-size)
      (setf (pkg-max-number-of-symbols pkg) size)
      )))

(defun rehash-intern-store (code sym length new-hash-table-codes new-hash-table-syms
                            &aux final-index)
...
;    (setf (symbol-package sym) pkg))
  (do ((i (rem code length) (rem (1+ i) length)))
      ((not (pkg-code-valid-p (svref new-hash-table-codes i)))
       (setq final-index i)
...
;                         (when (assoc sym c :test #'eq) (return t)))
...
;                 (push candidates external-conflicts))))))))
    (when (or local-conflicts inherited-conflicts external-conflicts)
      ;; Now report whatever conflicts we found.
      (error "USE-PACKAGE name conflict of symbols"))
...
(defun intern-symbol (sym &optional pkg &aux hash str)
  "Interns the string or symbol SYM in package PKG (or the current package).
If the package has a symbol whose pname matches SYM, that symbol is returned.
The USEd packages are also searched.
Otherwise, if SYM is a symbol, it is put in the package and returned.
    ...
    (let ((len (pkg-number-of-slots pkg))
      ...
      (do ((i (mod hash len)))
        ...
        (when (and (pkg-code-valid-p x)
                   (= hash (pkg-code-hash-code x))
                   (equal str (symbol-name (setq y (pkg-slot-symbol pkg i)))))
          (return-from intern-symbol (values y
                                      (if (pkg-code-external-p x)
                                          :external :internal)
                                      pkg)))
        (if (= (incf i) len) (setq i 0))))
