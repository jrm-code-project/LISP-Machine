...
;;Hold off on this benchmark until new compiler version is around to fix
;;compile of init.

;;COMPILE ON LAMBDA.
(defmacro char1 (x) `(array:aref-1 (string ,x) 0))
...
(defun gabriel-random () (setq rand (mod (* rand 17.) 251.)))

(defun gabriel-randomize (l)
       (do ((a ()))
           ((null l) a)
           (let ((n (mod (gabriel-random) (length l))))
                (cond ((= n 0)
                       (push (car l) a)
                       (setq l (cdr l)))
                      (t
...
(defun match (pat dat alist)
  (cond ((null pat)
        ...
        (t (cond ((atom (car pat))
                  (cond
                    ((eq (char1 (car pat)) #\?)
                     (let ((val (li:assoc (car pat) alist)))
