...
(defun alpha-combination (sym exp env &aux probe)
  (cond ((setq probe (table-entry *special-form-table* sym))
        ...
        ((setq probe (table-entry *primop-table* sym))
         (let ((arglist (primop.arglist probe)))
           (unless (or (member '&rest arglist)
                       (let ((length (length (cdr exp)))
                             (required (position '&optional arglist)))
                         (when required
                           (and (>= length required)
             ...
             (if (setq probe (get-subst sym))
                 ...
                 (cons
                   (let ((subst-name (if (consp (second probe))
                                         (car (second probe))
                                       (second probe))))
                     ;; Alphatizing the subst in the current environment
                     ;; makes the subst behave more like a macro than a function.
                     ;; The right thing would probably be to alphatise in the
                     ;; definition environment, but that could be hard to get
                     ;; ahold of.  The current behavior is more like ZetaLisp.
                     (alpha-lambda subst-name (third probe) (cdddr probe) env))
...
;;;      1 0
;;;      2 2 4
;;;      1 6))    ;; This line was missing, I think.  -smh 1jul88
;;;
  ...
  (cond ((or (not (symbolp name))
        ...
        (t
         (let ((var (obtain-variable env name)))
           ...
           ;; after we are sure the cold-load is working correctly
           ;; we can optimize this code by changing  (SYMBOL::SET ...) back to (SYMBOL::%%SET ...)
           ;; for now we really need to know if we are bashing NIL or T.
           ;; pfc 6/29   @@@
           (cond (var
                  (if (variable-special-p var)
...
(define-special-form IF (tested con . maybe-alt) (env)
  (when (consp (cdr maybe-alt))
    (error "multiple else forms handed to IF (if ~s ~s ~{~s~}) " tested con maybe-alt))
