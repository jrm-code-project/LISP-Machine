...
(defmacro primop-field (name args &body body)
  (let* ((title (concatenate-symbol 'primop. name))
         ...
         (primop (car args)))
    (push `(progn
             (setf (table-entry *primop-field-table* ',title) ',(intern name 'keyword))
             (defun ,title ,args
              (funcall (,field ,primop) . ,args)))
...
(defstruct (xprimop (:print-function (lambda (struct stream depth)
                                       ...
                                       (format stream "#{Primop ~s ~o}"
                                               (xprimop-id struct) (object-hash struct)))))
  handler
  bitv
  id
  (environment nil)
  ...
  source
  ;; Alphatize
  (make-closed (primop-field make-closed (primop) (make-closed-primop primop)))
  ;; Support
  (support     (primop-field support (primop call) nil))
  ;; Simplification
...
;  (#f ((identification self) (xprimop-id self))
...
(zl:defsubst primop.id (primop) (xprimop-id primop))
(zl:defsubst primop.environment (primop) (xprimop-environment primop))
(zl:defsubst primop.formals (primop) (xprimop-formals primop))
...
(defmacro define-primop-predicate (name)
  (let ((name (concatenate-symbol 'primop. name)))
    (prog1 `(progn
              (setf (table-entry *primop-predicate-table* ',name) ,*next-primop-predicate-index*)
...
(defmacro define-primop-operation (name args &rest default)
  (let* ((title (concatenate-symbol 'primop. name))
         (field (concatenate-symbol 'xprimop- name))
         (primop (car args)))
    `(progn (setf (table-entry *primop-operation-table* ',title) ',field)
            (defun ,title ,args
...
(define-primop-operation variant-id  (primop) (primop.id primop))
...
#||||||||||||||||||||||||||||||||
(defun make-primop (name bitv formals source env handler &rest fields)
  (let ((p (make-xprimop)))
    (set (xprimop-id          p) name)
    (set (xprimop-bitv        p) bitv)
    (set (xprimop-formals     p) formals)
    (set (xprimop-handler     p) handler)
    (set (xprimop-source      p) source)
    (set (xprimop-environment p) env)
