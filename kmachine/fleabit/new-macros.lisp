...
(defun get-subst (name)
  (get-declaration name 'prims:subst))


(defmacro ndefmacro (name args &body body)
  (let ((nname (gensym name)))
    `(progn
       (defmacro ,nname ,args
         ,@body)
       (setf (nlisp:macro-function ',name) (macro-function ',nname))
...
;; William had changed this in the ORSON version but not here on K-SYS:
;; Setting *EXPAND-HOST-MACROS-FOR-TARGET-P* to NIL makes DEFREWRITE work correctly.
;; --pfc 5/3/88
(defvar *expand-host-macros-for-target-p* nil
  "If this is set to to T then macros defined in the host environment
in which the compiler is running are expanded as if they were in the
target environment for which the compiler is compiling.")
      ...
      (cond ((not (atom name))
            ...
            ((setq expander (nlisp:macro-function name))
             ...
             (values (funcall nlisp:*macroexpand-hook* expander form environment)
                     t))
            ((and *expand-host-macros-for-target-p*
                  (setq expander (macro-function name)))
             ;; this can't really work because
             ;; this version of macroexpand uses a different representation
             ;; for the environment than lisp:macroexpand
             (values (funcall nlisp:*macroexpand-hook* expander form nil)))     ;environment
            (t (values form nil))))))
                     ...
                     (lambda (test-exp c)
                       (cond ((member c '(otherwise t))
                             ...
                             ((and (eq c ':otherwise)
                                   ...
                                   (member (car whole) '(selectq caseq)))
...
;; Added LI:NO-CASE-ERROR to K-SYS:COLD;ERROR.LISP.
;; Made the macros below generate calls to LI:NO-CASE-ERROR instead of NC:NO-CASE-ERROR.
;; ||| 27sept88 pfc
  ...
  (expand-case-macro 'etypecase place clauses
                     (lambda (test-exp c)
                       (if (member c '(otherwise t))
                         ...
                         `(li:typep ,test-exp ',c)))
                     (lambda (test-exp)
                       `(li:no-case-error nil 'etypecase ',place ,test-exp
           ...
           (return-from ,tag
             ,(expand-case-macro 'ctypecase place clauses
                                 ...
                                 (lambda (test-exp)
                                   `(progn
                                      (setf ,place
                                            (li:no-case-error t 'ctypecase ',place ,test-exp
                                                           '(or . ,(mapcar #'car clauses))))
                                      (go ,tag)))))))))
                     ...
                     (lambda (test-exp c)
                       (cond ((member c '(otherwise t))
                             ...
                             (t
                              `(member ,test-exp ',c))))
                     (lambda (test-exp)
                       `(li:no-case-error nil 'ecase ',place ,test-exp
                                       '(member
           ...
           (return-from ,tag
             ,(expand-case-macro 'ccase place clauses
                                 ...
                                 (lambda (test-exp)
                                   `(progn
                                      (setf ,place
                                            (li:no-case-error t 'ccase ',place ,test-exp
                                              '(member
                                                 . ,(mapcan (lambda (clause)
                                                              (let ((match (car clause)))
                                                                (if (not (consp match))
...
(ndefmacro li:check-type (place type &optional type-string)
...
If you omit it, it will be computed from TYPE."
  `(PROGN (UNLESS (LI:TYPEP ,place ',type)
...
;;; fake it for now
(ndefmacro li:without-interrupts (&body body)
  `(PROGN . ,body))
...
(defun variable-global-p (variable)
...
(ndefmacro prims::setup-initial-values-of-global-registers ()
