...
(defun format-[ (format-args directive-args colonp atsignp stream
  ...
  (cond (colonp  (format-colon-[ format-args directive-args
...
(defun format-colon-[ (format-args directive-args stream
                       format-string-iterator)
  (maximum-prefix-arguments 0 directive-args format-string-iterator)
  (let (false-string true-string)
    (multiple-value-bind (string ignore terminating-directive)
        (scan-a-chunk #\] format-string-iterator)
      (require-twiddle-semicolon terminating-directive)
      (setq false-string string))
    (multiple-value-bind (string ignore terminating-directive)
        (scan-a-chunk #\] format-string-iterator)
      (require-twiddle-] terminating-directive)
      (setq true-string string))
    (format-main (make-string-iterator (if (next-format-arg format-args)
                                           true-string
                                           false-string))
                 format-args
                 stream)))

(defun require-twiddle-semicolon (ignore))
(defun require-twiddle-] (ignore))
