...
(defvar *CTS-package-to-mailing-address-alist* NIL              ; $$$ New variable <18-Nov-88 smh>
...
;;; $$$ New function designed for top-level invocation from defsystem files. <18-Nov-88 smh>
...
(DEFUN SAVE-BUFFER (BUFFER &AUX FILE-ID PATHNAME (FILE-FILE-ID  NIL) cts-address)
  ...
  (AND (OR (SYMBOLP FILE-ID)
       ...
       (or (send buffer :get :NOT-CTS-CONTROLLED)
           ...
           ;; $$$ Reworked.  The problem is that when translations are used the losing pathname system can't
           ;; correlate the buffer's pathname with the one used by defsystem, so SYSTEMS-OF-PATHNAMES is
           ;; sometimes useless. <18-Nov-88 smh>
           ...
           (and (or (send buffer :get ':CTS-DONE)
                    (LET ((*QUERY-IO* *STANDARD-OUTPUT*))
                      (case (fquery '(:beep t
                        ...
                        (NIL NIL)
                        (T T))))
                ;; $$$ Send mail logging the changed sections.  This is new, tee-hee. <18-Nov-88 smh>
                (prog1
                  't
...
;;; Print Buffer sends a buffer to the Printer.  The buffer name is received from a minibuffer.
;;; Quick Print Buffer sends the current buffer to the Printer (use this on keys).
...
Offers to save it if it is a modified file buffer, unless NO-SAVE-P."
  ...
  ;; $$$ Replaced a three clause version of this test. <saz 11/15/88>
