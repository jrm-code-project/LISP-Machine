...
(defun cold-fasload (filespec &aux qfasl-binary-file fdefine-file-pathname *file-property-list*)
  ...
  (with-open-file (qfasl-binary-file filespec :direction :input
                                     :characters nil)
    (initialize-file-plist filespec)
    (setq fdefine-file-pathname (store-string 'sym:p-n-string
                                              (send (send filespec ':generic-pathname)
                                                    ':string-for-printing)))
    ;;;@@@Can't do the 8-bit check until FS:QFILE-INPUT-BINARY-STREAM screw on is
    ;;;fixed.  The next clause below is "correct" -- I added it to avoid
    ;;;weirdness if the attempt is made to cold-fasload a newfangled 8-bit binary
    ;;;-- but it was failing over the network. --Keith 26oct88
    ;;    ;;;Check for new 8-bit byte format:
    ;;    (let* ((size (or (send-if-handles qfasl-binary-file :byte-size)
    ;;               (send qfasl-binary-file :get :byte-size)))
    ;;     ;;;This is analogous to SI:FASLOAD-INTERNAL's  SI:*FASL-NIBBLE-8BIT* switch:
    ;;     (8bit (ecase size (8. t) (16. nil))))
    ;;      ;;;For now, we don't handle 8-bit binaries, we just error out:
    ;;      (and  8bit (ferror "Cold-Fasload does not handle 8-bit binary format")))
    ;;;Check magic nibbles:
    (or (and (= (qfasl-nibble) #o143150)
             (= (qfasl-nibble) #o71660))
        (ferror nil "~A is not a QFASL file" filespec))
    ;;I commented this out. Q-FASL-OP-VERSION-INFO is a noop now, but might need
    ;;to distinguish qfasl versions later (?) <02-Nov-88 keith>
    ;;;Unlike SI:FASLOAD-INTERNAL, we just ignore internal version info:
    ;    (when (= (logand (qfasl-nibble-peek) si:%fasl-group-type) si:fasl-op-version-info)
    ;      (qfasl-whack))
    ;;;Proceed to cold-fasload:
...
(defun qfasl-nibble-peek ()
  (send qfasl-binary-file ':tyipeek))
...
(defun m-fasl-op-package-symbol ()
  (do ((i 0 (1+ i))
       (path nil)
       ;; for making system 105 from system 104 sources.  Yuck!!!!!
       (cli-kludge (intern "CLI" sym-package))
       (sym)
      ...
      ((= i len)
       ...
       (cond ((> (length path) 2)
             ...
             (t
              (if (and (eq (car path) cli-kludge)
                       ;; fix 104-compiled qfasls.
                       (or (eq (cadr path) (intern "ERROR" sym-package))
                           (eq (cadr path) (intern "TERPRI" sym-package))))
                  (progn (format t "~&[Interned CLI:~A]" (cadr path))
                         (m-enter-fasl-table (cadr path)))
...
(defun q-fasl-convert-fef-to-fast-fef-if-possible (fef)
  (let ((header-word (vread (+ fef sym:%fefhi-ipc)))
        (fast-arg-word (vread (+ fef sym:%fefhi-fast-arg-opt)))
        (length-word (vread (+ fef sym:%fefhi-storage-length)))
        (misc-word (vread (+ fef sym:%fefhi-misc))))
    (flet ((rewrite-fef-description (header-word fast-arg-word length-word misc-word)
             (vwrite (+ fef sym:%fefhi-ipc) header-word)
             (vwrite (+ fef sym:%fefhi-fast-arg-opt) fast-arg-word)
             (vwrite (+ fef sym:%fefhi-storage-length) length-word)
             (vwrite (+ fef sym:%fefhi-misc) misc-word)))
      (cond ((= (ldb sym:%%header-type-field header-word) sym:%header-type-fef)
             (let ((get-self-mapping-table (ldb sym:%%fefh-get-self-mapping-table header-word))
                   (sv-bind (ldb sym:%%fefh-sv-bind header-word))
                   (fast-arg (ldb sym:%%fefh-fast-arg header-word))
                   (no-adl (ldb sym:%%fefh-no-adl header-word))
                   (min-args (ldb sym:%%arg-desc-min-args fast-arg-word))
                   (max-args (ldb sym:%%arg-desc-max-args fast-arg-word))
                   (local-block (ldb sym:%%fefhi-ms-local-block-length misc-word)))
               (cond ((or (not (zerop get-self-mapping-table))
                          (not (zerop sv-bind))
                          (not (= fast-arg 1))
                          (bit-test sym:%arg-desc-quoted-rest fast-arg-word)
                          (bit-test sym:%arg-desc-evaled-rest fast-arg-word)
                          (bit-test sym:%arg-desc-interpreted fast-arg-word)
                          (bit-test sym:%arg-desc-fef-quote-hair fast-arg-word)
                          (bit-test sym:%arg-desc-fef-bind-hair fast-arg-word))
                      nil)
                     ((and (= min-args max-args)
                           (zerop local-block)
                           ( min-args (dpb -1 (logand 77 sym:%%fefh-args-for-fanl) 0)))
                      ;; Fixed number of arguments, no local block.
                      (rewrite-fef-description
                        (dpb min-args
                             sym:%%fefh-args-for-fanl
                             (dpb sym:%header-type-fast-fef-fixed-args-no-locals
                                  sym:%%header-type-field
                                  header-word))
                        fast-arg-word
                        (dpb no-adl sym:%%fefsl-no-adl length-word)
                        misc-word))
                     ((and (zerop local-block)
                           ( min-args (dpb -1 (logand 77 sym:%%fefh-min-args-for-vanl) 0))
                           ( max-args (dpb -1 (logand 77 sym:%%fefh-max-args-for-vanl) 0)))
                      ;; Variable number of arguments, no local block.
                      (rewrite-fef-description
                        (dpb min-args
                             sym:%%fefh-min-args-for-vanl
                             (dpb max-args
                                  sym:%%fefh-max-args-for-vanl
                                  (dpb sym:%header-type-fast-fef-var-args-no-locals
                                       sym:%%header-type-field
                                       header-word)))
                        fast-arg-word
                        (dpb no-adl sym:%%fefsl-no-adl length-word)
                        misc-word))
                     ((and (= min-args max-args)
                           ( min-args (dpb -1 (logand 77 sym:%%fefh-args-for-fawl) 0))
                           ( local-block (dpb -1 (logand 77 sym:%%fefh-locals-for-fawl) 0)))
                      ;; Fixed number of arguments with local block.
                      (rewrite-fef-description
                        (dpb min-args
                             sym:%%fefh-args-for-fawl
                             (dpb local-block
                                  sym:%%fefh-locals-for-fawl
                                  (dpb sym:%header-type-fast-fef-fixed-args-with-locals
                                       sym:%%header-type-field
                                       header-word)))
                        fast-arg-word
                        (dpb no-adl sym:%%fefsl-no-adl length-word)
                        misc-word))
                     ((and ( min-args (dpb -1 (logand 77 sym:%%fefh-min-args-for-vawl) 0))
                           ( max-args (dpb -1 (logand 77 sym:%%fefh-max-args-for-vawl) 0))
                           ( local-block (dpb -1 (logand 77 sym:%%fefh-locals-for-vawl) 0)))
                      ;; Variable number of arguments with local block.
                      (rewrite-fef-description
                        (dpb min-args
                             sym:%%fefh-min-args-for-vawl
                             (dpb max-args
                                  sym:%%fefh-max-args-for-vawl
                                  (dpb local-block
                                       sym:%%fefh-locals-for-vawl
                                       (dpb sym:%header-type-fast-fef-var-args-with-locals
                                            sym:%%header-type-field
                                            header-word))))
                        fast-arg-word
                        (dpb no-adl sym:%%fefsl-no-adl length-word)
                     ...
                     (t
                      nil))))
            (t
             (ferror () "Expected a FEF header at location ~O" (logand q-pointer-mask fef)))))))
...
;;; $$$ Provide for reading (ignoring) new version info group <27-Oct-88 keith>

(defun q-fasl-op-version-info ()
  (q-fasl-next-value)
  (q-fasl-next-value))
